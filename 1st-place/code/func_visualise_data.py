...
def mov_sum(df, past, future):
    ...
    for i in range(past):
        new_df = new_df.add(np.r_[npa[:(i+1)], npa[:-(i+1)]], fill_value=0)
        
    for i in range(future):
        new_df = new_df.add(np.r_[npa[(i+1):], npa[-(i+1):]], fill_value=0)
        
    new_df = new_df / (past + future + 1)
...
class Sequence(Slicer):
    ...
    def rssi_movSum(self, past, future):
        df = self.rssi
        if df.shape[0] < min(1, past+1, future+1):
            return None
        npa = df.values
        npa[np.isnan(npa)] = 0
        new_npa = npa
        ...
        for i in range(past):
            new_npa = new_npa + np.r_[npa[:(i+1)], npa[:-(i+1)]]
        for i in range(future):
            new_npa = new_npa + np.r_[npa[(i+1):], npa[-(i+1):]]
        new_npa = new_npa / (past + future + 1)
        new_npa[new_npa==0] = np.nan
        
        new_df = pd.DataFrame(new_npa)
        new_df = new_df.set_index(df.index)
        new_df.columns = df.columns        
        
        self.rssi = new_df

    def accel_movSum(self, past, future):
        df = self.acceleration
        if df.shape[0] < min(1, past+1, future+1):
            return None
        npa = df.values
        npa[np.isnan(npa)] = 0
        ...
        self.acceleration = new_df
    
    def video_movSum(self, past, future):
        df = self.video['living_room']
        if df.shape[0] < min(1, past+1, future+1):
            return None
        npa = df.values
        npa[np.isnan(npa)] = 0
        ...
        self.video['living_room'] = new_df
        
        df = self.video['kitchen']
        if df.shape[0] < min(1, past+1, future+1):
            return None
        npa = df.values
        npa[np.isnan(npa)] = 0
        ...
        self.video['kitchen'] = new_df
        
        df = self.video['hallway']
        if df.shape[0] < min(1, past+1, future+1):
            return None
        npa = df.values
        npa[np.isnan(npa)] = 0
        ...
        self.video['hallway'] = new_df
        

    def iterate(self, step=1): #step in seconds, float [1,0)
        start = np.arange(0,int(self.meta['end']) + step,step)
        end = (start + 1).tolist()
        start = start.tolist()
    ...
    def iterate_v1(self, step=1): #step in seconds, float [1,0)
        start = np.arange(0,int(self.meta['end']) + step,step)
        end = (start + 1).tolist()
        start = start.tolist()
        ...
        for lower, upper in zip(start, end):
            ...
            # PIR
            lu_step = (lu[1] - lu[0]) / 10.0
            pir_df *= 0.0
            for i_row, i_lower in enumerate(np.arange(lu[0], lu[1], lu_step)):
                pir_start_stop = slice_df_start_stop(self.pir, (i_lower, i_lower + lu_step))
                if pir_start_stop.shape[0] > 0:
                    for si, series in pir_start_stop.iterrows():
                        pir_df.set_value(pir_t[i_row], series['name'], 1.0)
            pir_t += 1
    ...
    def iterate_location(self, step=1): #step in seconds, float [1,0)
        start = np.arange(0,int(self.meta['end']) + step,step)
        end = (start + 1).tolist()
        start = start.tolist()

        loc_zeros = [np.zeros(1)] * len(self.location_targets)
        loc_t = np.linspace(0, 1, 1, endpoint=False)
        loc_df = pd.DataFrame(dict(zip(self.location_targets, loc_zeros)))
        loc_df['t'] = loc_t
        loc_df.set_index('t', inplace=True)
        ...
        for lower, upper in zip(start, end):
            ...
            for i in range(len(self.locations)):
                loc_start_stop = slice_df_start_stop(self.locations[i], lu)
                if loc_start_stop.shape[0] > 0:
                    for si, series in loc_start_stop.iterrows():
                        loc_df[series['name']] = loc_df[series['name']] + 1.0
                loc_df = loc_df / len(self.locations)
            
            loc_t += 1
            
            yield lu, (loc_df.copy())

    def iterate_annotation(self, step=1): #step in seconds, float [1,0)
        start = np.arange(0,int(self.meta['end']) + step,step)
        end = (start + 1).tolist()
        start = start.tolist()

        loc_zeros = [np.zeros(1)] * len(self.activity_targets)
        loc_t = np.linspace(0, 1, 1, endpoint=False)
        loc_df = pd.DataFrame(dict(zip(self.activity_targets, loc_zeros)))
        loc_df['t'] = loc_t
        loc_df.set_index('t', inplace=True)
        ...
        for lower, upper in zip(start, end):
            ...
            for i in range(len(self.annotations)):
                loc_start_stop = slice_df_start_stop(self.annotations[i], lu)
                if loc_start_stop.shape[0] > 0:
                    for si, series in loc_start_stop.iterrows():
                        loc_df[series['name']] = loc_df[series['name']] + min((min(series['end'], upper) - 
                                                 max(series['start'], lower)), 1)
