...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+1)):
        for i2 in ['2Df1','2Df2','2Df3','2Df4','2Df5','2Df6']:
            for i3 in ['dif','slo']:
                column_names_A.append('video_sta_M10_{0}_{1}of1_{2}_{3}'.format(i0, 'v2', i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+1)):
        for i2 in ['3Df1','3Df2','3Df3','3Df4','3Df5','3Df6','3Df7','3Df8','3Df9']:
            for i3 in ['dif','slo']:
                column_names_B.append('video_sta_M10_{0}_{1}of1_{2}_{3}'.format(i0, 'v2', i2, i3)) 
...
for train_test in ('train', 'test', ): 
    ...
    for fi, file_id in enumerate(sorted(os.listdir('../public_data/{}/'.format(train_test)))):
        ...
        #### Generate ds_video_sta_M10_v1A/B (2 files)
        ...
        for ri, (lu, (accel, rssi, pir, vid_lr, vid_k, vid_h)) in enumerate(data.iterate()):
            ...
            row = np.array([])
            row = np.append(row, extract_video_sta_v2A(vid_lr.copy()))
            row = np.append(row, extract_video_sta_v2A(vid_k.copy()))
            row = np.append(row, extract_video_sta_v2A(vid_h.copy()))
            rows_A.append(row)
            
            row = np.array([])
            row = np.append(row, extract_video_sta_v2B(vid_lr.copy()))
            row = np.append(row, extract_video_sta_v2B(vid_k.copy()))
            row = np.append(row, extract_video_sta_v2B(vid_h.copy()))
            rows_B.append(row)
        ...
        df_A.to_csv('{}ds_video_sta_M10_v2A.csv'.format(directory), index=False)
        
        df_B = pd.DataFrame(rows_B)
        df_B.columns = column_names_B
        df_B.to_csv('{}ds_video_sta_M10_v2B.csv'.format(directory), index=False)
