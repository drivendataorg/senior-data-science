...
def extract_video_sta_v1A(df, n_split):
    '''
    ...
    return: numpy 2D-array of new features:
        [mean, std, min, median, max] of [2D features] for each 'n_splits' group
    '''
    num_ff = (6 + 0) * n_split * 5  # Number of final features
...
def extract_video_sta_v1B(df, n_split):
    '''
    This function extract features from collected 'video' data.
    df: pandas dataFrame of data for a given time-window (1s)
    n_split: int[1,] number of groups inside the given time-window
    return: numpy 2D-array of new features:
        [mean, std, min, median, max] of [3D features] for each 'n_splits' group
    '''
    num_ff = (9 + 0) * n_split * 5  # Number of final features
    ...
    for i1 in range(1,(1+1)):
        for i2 in ['2Df1','2Df2','2Df3','2Df4','2Df5','2Df6']:
            for i3 in ['mean','std','min','median','max',]:
                column_names_A_s1.append('video_sta_M10_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+1)):
        for i2 in ['3Df1','3Df2','3Df3','3Df4','3Df5','3Df6','3Df7','3Df8','3Df9']:
            for i3 in ['mean','std','min','median','max',]:
                column_names_B_s1.append('video_sta_M10_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+2)):
        for i2 in ['2Df1','2Df2','2Df3','2Df4','2Df5','2Df6']:
            for i3 in ['mean','std','min','median','max',]:
                column_names_A_s2.append('video_sta_M10_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+2)):
        for i2 in ['3Df1','3Df2','3Df3','3Df4','3Df5','3Df6','3Df7','3Df8','3Df9']:
            for i3 in ['mean','std','min','median','max',]:
                column_names_B_s2.append('video_sta_M10_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for train_test in ('train', 'test', ): 
    ...
    for fi, file_id in enumerate(sorted(os.listdir('../public_data/{}/'.format(train_test)))):
        ...
        #### Generate ds_video_sta_M06_v1A/B_s1 / ds_video_sta_M06_v1A/B_s2 (4 files)
        ...
        data.load()
        data.video_movSum(10,10) # Transform video data to mov_sum 
        ...
        for ri, (lu, (accel, rssi, pir, vid_lr, vid_k, vid_h)) in enumerate(data.iterate()):
            ...
            row = np.array([])
            row = np.append(row, extract_video_sta_v1A(vid_lr.copy(), 1))
            row = np.append(row, extract_video_sta_v1A(vid_k.copy(), 1))
            row = np.append(row, extract_video_sta_v1A(vid_h.copy(), 1))
            rows_A_s1.append(row)
            
            row = np.array([])
            row = np.append(row, extract_video_sta_v1A(vid_lr.copy(), 2))
            row = np.append(row, extract_video_sta_v1A(vid_k.copy(), 2))
            row = np.append(row, extract_video_sta_v1A(vid_h.copy(), 2))
            rows_A_s2.append(row)
            
            row = np.array([])
            row = np.append(row, extract_video_sta_v1B(vid_lr.copy(), 1))
            row = np.append(row, extract_video_sta_v1B(vid_k.copy(), 1))
            row = np.append(row, extract_video_sta_v1B(vid_h.copy(), 1))
            rows_B_s1.append(row)
            
            row = np.array([])
            row = np.append(row, extract_video_sta_v1B(vid_lr.copy(), 2))
            row = np.append(row, extract_video_sta_v1B(vid_k.copy(), 2))
            row = np.append(row, extract_video_sta_v1B(vid_h.copy(), 2))
        ...
        df_A_s1.to_csv('{}ds_video_sta_M10_v1A_s1.csv'.format(directory), index=False)
        
        df_A_s2 = pd.DataFrame(rows_A_s2)
        df_A_s2.columns = column_names_A_s2
        df_A_s2.to_csv('{}ds_video_sta_M10_v1A_s2.csv'.format(directory), index=False)
        
        df_B_s1 = pd.DataFrame(rows_B_s1)
        df_B_s1.columns = column_names_B_s1
        df_B_s1.to_csv('{}ds_video_sta_M10_v1B_s1.csv'.format(directory), index=False)
        
        df_B_s2 = pd.DataFrame(rows_B_s2)
        df_B_s2.columns = column_names_B_s2
        df_B_s2.to_csv('{}ds_video_sta_M10_v1B_s2.csv'.format(directory), index=False)
