...
# Add past data
dataset = {'train_x':np.c_[all_train_x, fd.get_past_data(all_train_x, train_seq, 1, 0,200),
                           fd.get_past_data(all_train_x, train_seq, 2, 0,200),
                           fd.get_future_data(all_train_x, train_seq, 1, 0,200)], 
            'train_y':all_train_y, 
            'train_seq':train_seq, 
...
def predict_model(train_x, train_y, test_x, test_y=None, class_weights=None, random_state=0):
    ...
    model = fp.PMC_NeuralNetwork_T1(train_x.shape[1], train_y.shape[1], params, bags=5) 
