    ...
    for i1 in range(1,(1+1)):
        for i2 in ['2Df1','2Df2','2Df3','2Df4','2Df5','2Df6']:
            for i3 in ['mean','std','min','median','max','sum','asum']:
                column_names_A_s1.append('video_mov_M00_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+1)):
        for i2 in ['3Df1','3Df2','3Df3','3Df4','3Df5','3Df6','3Df7','3Df8','3Df9']:
            for i3 in ['mean','std','min','median','max','sum','asum']:
                column_names_B_s1.append('video_mov_M00_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+2)):
        for i2 in ['2Df1','2Df2','2Df3','2Df4','2Df5','2Df6']:
            for i3 in ['mean','std','min','median','max','sum','asum']:
                column_names_A_s2.append('video_mov_M00_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+2)):
        for i2 in ['3Df1','3Df2','3Df3','3Df4','3Df5','3Df6','3Df7','3Df8','3Df9']:
            for i3 in ['mean','std','min','median','max','sum','asum']:
                column_names_B_s2.append('video_fmov_M00_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for train_test in ('train', 'test', ): 
    ...
    for fi, file_id in enumerate(sorted(os.listdir('../public_data/{}/'.format(train_test)))):
        ...
        #### Generate ds_video_mov_M00_v1A/B_s1 / ds_video_mov_M00_v1A/B_s2 (4 files)
        ...
        df_A_s1.to_csv('{}ds_video_mov_M00_v1A_s1.csv'.format(directory), index=False)
        
        df_A_s2 = pd.DataFrame(rows_A_s2)
        df_A_s2.columns = column_names_A_s2
        df_A_s2.to_csv('{}ds_video_mov_M00_v1A_s2.csv'.format(directory), index=False)
        
        df_B_s1 = pd.DataFrame(rows_B_s1)
        df_B_s1.columns = column_names_B_s1
        df_B_s1.to_csv('{}ds_video_mov_M00_v1B_s1.csv'.format(directory), index=False)
        
        df_B_s2 = pd.DataFrame(rows_B_s2)
        df_B_s2.columns = column_names_B_s2
        df_B_s2.to_csv('{}ds_video_mov_M00_v1B_s2.csv'.format(directory), index=False)
