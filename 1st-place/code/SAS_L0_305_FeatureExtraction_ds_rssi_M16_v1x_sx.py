...
for train_test in ('train', 'test', ): 
    ...
    for fi, file_id in enumerate(sorted(os.listdir('../public_data/{}/'.format(train_test)))):
        ...
        data.load()
        data.rssi_movSum(16,16) # Transform rssi data to mov_sum
        ...
        df_A_s1.to_csv('{}ds_rssi_M16_v1A_s1.csv'.format(directory), index=False)
        
        df_A_s2 = pd.DataFrame(rows_A_s2)
        df_A_s2.columns = column_names_A_s2
        df_A_s2.to_csv('{}ds_rssi_M16_v1A_s2.csv'.format(directory), index=False)
        
        df_B_s1 = pd.DataFrame(rows_B_s1)
        df_B_s1.columns = column_names_B_s1
        df_B_s1.to_csv('{}ds_rssi_M16_v1B_s1.csv'.format(directory), index=False)
        
        df_B_s2 = pd.DataFrame(rows_B_s2)
        df_B_s2.columns = column_names_B_s2
        df_B_s2.to_csv('{}ds_rssi_M16_v1B_s2.csv'.format(directory), index=False)
        
        df_C_s1 = pd.DataFrame(rows_C_s1)
        df_C_s1.columns = column_names_C_s1
        df_C_s1.to_csv('{}ds_rssi_M16_v1C_s1.csv'.format(directory), index=False)
        
        df_C_s2 = pd.DataFrame(rows_C_s2)
        df_C_s2.columns = column_names_C_s2
        df_C_s2.to_csv('{}ds_rssi_M16_v1C_s2.csv'.format(directory), index=False)
        
        df_D_s1 = pd.DataFrame(rows_D_s1)
        df_D_s1.columns = column_names_D_s1
        df_D_s1.to_csv('{}ds_rssi_M16_v1D_s1.csv'.format(directory), index=False)
        
        df_D_s2 = pd.DataFrame(rows_D_s2)
        df_D_s2.columns = column_names_D_s2
        df_D_s2.to_csv('{}ds_rssi_M16_v1D_s2.csv'.format(directory), index=False)
        
        df_E_s1 = pd.DataFrame(rows_E_s1)
        df_E_s1.columns = column_names_E_s1
        df_E_s1.to_csv('{}ds_rssi_M16_v1E_s1.csv'.format(directory), index=False)
        
        df_E_s2 = pd.DataFrame(rows_E_s2)
        df_E_s2.columns = column_names_E_s2
        df_E_s2.to_csv('{}ds_rssi_M16_v1E_s2.csv'.format(directory), index=False)
        
        df_F_s1 = pd.DataFrame(rows_F_s1)
        df_F_s1.columns = column_names_F_s1
        df_F_s1.to_csv('{}ds_rssi_M16_v1F_s1.csv'.format(directory), index=False)
        
        df_F_s2 = pd.DataFrame(rows_F_s2)
        df_F_s2.columns = column_names_F_s2
        df_F_s2.to_csv('{}ds_rssi_M16_v1F_s2.csv'.format(directory), index=False)
