    ...
    for i1 in range(1,(1+1)):
        for i2 in ['d2_width','d2_height','d2_sup','d2_ratio']:
            for i3 in ['mean','std','min','median','max',]:
                column_names_A_s1.append('video_fig_M06_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+1)):
        for i2 in ['d3_width','d3_height','d3_depth','d3_wh_sup',
            ...
            for i3 in ['mean','std','min','median','max',]:
                column_names_B_s1.append('video_fig_M06_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+2)):
        for i2 in ['d2_width','d2_height','d2_sup','d2_ratio']:
            for i3 in ['mean','std','min','median','max',]:
                column_names_A_s2.append('video_fig_M06_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for i0 in ['vid_lr','vid_k','vid_h']:
    for i1 in range(1,(1+2)):
        for i2 in ['d3_width','d3_height','d3_depth','d3_wh_sup',
                           'd3_wd_sup','d3_hd_sup','d3_wh_ratio','d3_wd_ratio','d3_hd_ratio','d3_vol']:
            for i3 in ['mean','std','min','median','max',]:
                column_names_B_s2.append('video_fig_M06_{0}_{1}of1_{2}_{3}'.format(i0, i1, i2, i3)) 
...
for train_test in ('train', 'test', ): 
    ...
    for fi, file_id in enumerate(sorted(os.listdir('../public_data/{}/'.format(train_test)))):
        ...
        #### Generate ds_video_fig_M06_v1A/B_s1 / ds_video_fig_M06_v1A/B_s2 (4 files)
        ...
        df_A_s1.to_csv('{}ds_video_fig_M06_v1A_s1.csv'.format(directory), index=False)
        
        df_A_s2 = pd.DataFrame(rows_A_s2)
        df_A_s2.columns = column_names_A_s2
        df_A_s2.to_csv('{}ds_video_fig_M06_v1A_s2.csv'.format(directory), index=False)
        
        df_B_s1 = pd.DataFrame(rows_B_s1)
        df_B_s1.columns = column_names_B_s1
        df_B_s1.to_csv('{}ds_video_fig_M06_v1B_s1.csv'.format(directory), index=False)
        
        df_B_s2 = pd.DataFrame(rows_B_s2)
        df_B_s2.columns = column_names_B_s2
        df_B_s2.to_csv('{}ds_video_fig_M06_v1B_s2.csv'.format(directory), index=False)
