...
for train_test in ('train', 'test', ): 
    ...
    for fi, file_id in enumerate(sorted(os.listdir('../public_data/{}/'.format(train_test)))):
        ...
        #### Generate ds_accel_M03_v2A/B/C/D (4 files)
        ...
        data.load()
        data.accel_movSum(3,3) # Transform accel data to mov_sum   
        ...
        df_A.to_csv('{}ds_accel_M03_v2A.csv'.format(directory), index=False)
        
        df_B = pd.DataFrame(rows_B)
        df_B.columns = column_names_B
        df_B.to_csv('{}ds_accel_M03_v2B.csv'.format(directory), index=False)
        
        df_C = pd.DataFrame(rows_C)
        df_C.columns = column_names_C
        df_C.to_csv('{}ds_accel_M03_v2C.csv'.format(directory), index=False)
        
        df_D = pd.DataFrame(rows_D)
        df_D.columns = column_names_D
        df_D.to_csv('{}ds_accel_M03_v2D.csv'.format(directory), index=False)
