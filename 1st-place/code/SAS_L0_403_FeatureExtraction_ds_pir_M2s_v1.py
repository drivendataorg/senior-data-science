...
for train_test in ('train', 'test', ): 
    ...
    for fi, file_id in enumerate(sorted(os.listdir('../public_data/{}/'.format(train_test)))):
        ...
        # Normalize data using 2s-future & 2s-past rows
        rows = rows + np.r_[rows[1:], rows[-1:]] + np.r_[rows[2:], rows[-2:]] + np.r_[rows[:1], rows[:-1]] + np.r_[rows[:2], rows[:-2]]
        ...
        df.to_csv('{}ds_pir_M2s_v1.csv'.format(directory), index=False)
