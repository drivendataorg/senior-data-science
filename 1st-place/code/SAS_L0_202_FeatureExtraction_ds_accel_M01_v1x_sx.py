...
for i1 in range(1,(1+1)):
    for i2 in ['x','y','z']:
        for i3 in ['mean','std','min','median','max',]:
            column_names_A_s1.append('{0}_{1}of1_{2}_{3}'.format('accel_M01', i1, i2, i3)) 

column_names_B_s1 = []
for i1 in range(1,(1+1)):
    for i2 in ['comp','max_id','max','min_id','min']:
        for i3 in ['mean','std','min','median','max',]:
            column_names_B_s1.append('{0}_{1}of1_{2}_{3}'.format('accel_M01', i1, i2, i3)) 

column_names_A_s2 = []
for i1 in range(1,(1+2)):
    for i2 in ['x','y','z']:
        for i3 in ['mean','std','min','median','max',]:
            column_names_A_s2.append('{0}_{1}of2_{2}_{3}'.format('accel_M01', i1, i2, i3)) 

column_names_B_s2 = []
for i1 in range(1,(1+2)):
    for i2 in ['comp','max_id','max','min_id','min']:
        for i3 in ['mean','std','min','median','max',]:
            column_names_B_s2.append('{0}_{1}of2_{2}_{3}'.format('accel_M01', i1, i2, i3))  
...
for train_test in ('train', 'test', ): 
    ...
    for fi, file_id in enumerate(sorted(os.listdir('../public_data/{}/'.format(train_test)))):
        ...
        #### Generate ds_accel_M01_v1A/B_s1 / ds_accel_M01_v1A/B_s2 (4 files)
        ...
        data.load()
        data.accel_movSum(1,1) # Transform accel data to mov_sum        
        ...
        df_A_s1.to_csv('{}ds_accel_M01_v1A_s1.csv'.format(directory), index=False)
        
        df_A_s2 = pd.DataFrame(rows_A_s2)
        df_A_s2.columns = column_names_A_s2
        df_A_s2.to_csv('{}ds_accel_M01_v1A_s2.csv'.format(directory), index=False)
        
        df_B_s1 = pd.DataFrame(rows_B_s1)
        df_B_s1.columns = column_names_B_s1
        df_B_s1.to_csv('{}ds_accel_M01_v1B_s1.csv'.format(directory), index=False)
        
        df_B_s2 = pd.DataFrame(rows_B_s2)
        df_B_s2.columns = column_names_B_s2
        df_B_s2.to_csv('{}ds_accel_M01_v1B_s2.csv'.format(directory), index=False)
