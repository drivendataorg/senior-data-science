...
def predict(model, filename, isXgb=False):
    ...
    with open(filename, 'w') as fil:
        ...
        for te_ind_str in sorted(os.listdir(os.path.join('../input/public_data', 'test'))):
            ...
            features = np.hstack((features, features1, features2, features3, features4, features5, features6))
...
def xgb_model(train_x, train_y):
    ...
    np.savetxt("../models/xgb_v21.val.txt", val_preds)
...
def main():
    ...
    predict(clf, "../sub/xgb_v21.tst.csv", True)
    predict(models, "../sub/xgb_v21.valtst.csv", True)
