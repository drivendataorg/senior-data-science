...
def load_sequence(file_id):
    ...
    df = pd.read_csv('../input/public_data/train/{}/columns_v4.csv'.format(filename))
...
def predict(model, filename, isXgb=False):
    ...
    with open(filename, 'w') as fil:
        ...
        for te_ind_str in sorted(os.listdir(os.path.join('../input/public_data', 'test'))):
            ...
            features = pd.read_csv(os.path.join('../input/public_data', 'test', te_ind_str, 'columns_v4.csv')).values
...
def xgb_model(train_x, train_y):
    ...
    np.savetxt("../models/xgb_v8.val.txt", val_preds)
...
def main():
    ...
    predict(clf, "../sub/xgb_v8.tst.csv", True)
