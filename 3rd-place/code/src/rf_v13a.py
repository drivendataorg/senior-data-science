...
def rf_model(train_x, train_y):
    ...
    np.savetxt("../models/rf_v13.val.txt", val_preds)    

    classifier = ensemble.RandomForestClassifier(n_jobs=-1, n_estimators=2000, max_features=20, min_samples_split=3,
                                    bootstrap=True, verbose=1, random_state=2016, class_weight= dict(enumerate(class_weights)))
...
def main():
    ...
    clf, models, score = rf_model(train_x, train_y)
    logging.info("Predicting test data - " + str(datetime.datetime.now()))
    predict(clf, "../sub/rf_v13.tst.csv", False)
