...
def load_sequence(file_id):
    ...
    #df4 = pd.read_csv('../input/public_data/train/{}/columns_v13.csv'.format(filename))
...
def load_data():
    ...
    train_x7 = np.zeros(train_x.shape)
    train_x7[-1,:] = -1
    train_x7[:-1,:] = train_x4[1:,:]
    
    train_x8 = np.zeros(train_x.shape)
    train_x8[-1,:] = -1
    train_x8[:-1,:] = train_x6[1:,:]
    
    train_x = np.hstack((train_x, train_x1, train_x2, train_x3, train_x4, train_x5, train_x6, train_x7, train_x8))
...
def predict(model, filename, isXgb=False):
    ...
    with open(filename, 'w') as fil:
        ...
        for te_ind_str in sorted(os.listdir(os.path.join('../input/public_data', 'test'))):
            ...
            #features_v12 = pd.read_csv(os.path.join('../input/public_data', 'test', te_ind_str, 'columns_v13.csv')).values
            ...
            features5 = np.zeros(features.shape)
            features5[0,:] = -1
            features5[1:,:] = features2[:-1,:]

            features6 = np.zeros(features.shape)
            features6[-1,:] = -1
            features6[:-1:,:] = features4[1:,:]
            
            features7 = np.zeros(features.shape)
            features7[-1,:] = -1
            features7[:-1:,:] = features4[1:,:]

            features8 = np.zeros(features.shape)
            features8[-1,:] = -1
            features8[:-1:,:] = features6[1:,:]
            
            features = np.hstack((features, features1, features2, features3, features4, features5, features6, features7, features8))
...
def xgb_model(train_x, train_y):
    ...
    np.savetxt("../models/xgb_v22.val.txt", val_preds)
...
def main():
    ...
    predict(clf, "../sub/xgb_v22.tst.csv", True)
    predict(models, "../sub/xgb_v22.valtst.csv", True)
