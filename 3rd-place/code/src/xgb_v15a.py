...
def load_sequence(file_id):
    ...
    df5 = pd.read_csv('../input/public_data/train/{}/columns_v11.csv'.format(filename))
    
    data = np.hstack((df1.values, df2.values, df3.values, df5.values))
...
def predict(model, filename, isXgb=False):
    ...
    with open(filename, 'w') as fil:
        ...
        for te_ind_str in sorted(os.listdir(os.path.join('../input/public_data', 'test'))):
            ...
            features_v10 = pd.read_csv(os.path.join('../input/public_data', 'test', te_ind_str, 'columns_v11.csv')).values

            features = np.hstack((features_v5, features_v7, features_v8, features_v10))
...
def xgb_model(train_x, train_y):
    ...
    for i in range(kfold):
         ...
         # Training
         tic = time.time()
         xg_train = xgb.DMatrix(X_train, label=np.argmax(y_train, axis=1))
    ...
    np.savetxt("../models/xgb_v15.val.txt", val_preds)
...
def main():
    ...
    predict(clf, "../sub/xgb_v15.tst.csv", True)
    predict(models, "../sub/xgb_v15.valtst.csv", True)
