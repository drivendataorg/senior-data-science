...
def load_sequence(file_id):
    ...
    df16 = pd.read_csv('../input/public_data/train/{}/columns_v16.csv'.format(filename))
    
    data = np.hstack((df1.values, df2.values, df3.values, df16.values))
...
def predict(model, filename, isXgb=False):
    ...
    with open(filename, 'w') as fil:
        ...
        for te_ind_str in sorted(os.listdir(os.path.join('../input/public_data', 'test'))):
            ...
            features_v16 = pd.read_csv(os.path.join('../input/public_data', 'test', te_ind_str, 'columns_v16.csv')).values

            features = np.hstack((features_v5, features_v7, features_v8, features_v16))
...
def xgb_model(train_x, train_y):
    ...
    np.savetxt("../models/xgb_v24.val.txt", val_preds)
...
def main():
    ...
    predict(clf, "../sub/xgb_v24.tst.csv", True)
